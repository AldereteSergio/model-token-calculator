...
const INITIAL_MODELS = [
  { id: '1', name: 'Gemini 1.5 Flash', inputPrice: 0.075, outputPrice: 0.30, cachePrice: 0.01875 },
  { id: '2', name: 'Gemini 1.5 Pro', inputPrice: 1.25, outputPrice: 5.00, cachePrice: 0.3125 },
  { id: '3', name: 'GPT-4o', inputPrice: 2.50, outputPrice: 10.00, cachePrice: 1.25 },
  { id: '4', name: 'GPT-4o mini', inputPrice: 0.15, outputPrice: 0.60, cachePrice: 0.075 },
  { id: '5', name: 'Claude 3.5 Sonnet', inputPrice: 3.00, outputPrice: 15.00, cachePrice: 0.30 }
];

export default function App() {
  const [csvData, setCsvData] = useState(INITIAL_CSV_DATA);
  const [models, setModels] = useState(INITIAL_MODELS);
  const [selectedModelId, setSelectedModelId] = useState('1'); // Gemini 1.5 Flash por defecto
  ...
  const mostExpensiveWeek = useMemo(() => {
    ...
    let maxCost = -1;
    let bestWeek = 'N/A';
    let bestWeekData = { cost: 0, totalTokens: 0 };
  ...
  const handleAddOrUpdateModel = (e) => {
    ...
    if (isEditing) {
      ...
      const newModel = {
        ...
        name: modelForm.name,
        inputPrice: parseFloat(modelForm.inputPrice) || 0,
        outputPrice: parseFloat(modelForm.outputPrice) || 0,
        cachePrice: parseFloat(modelForm.cachePrice) || 0
      };
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans p-4 md:p-8 selection:bg-cyan-500 selection:text-slate-900">
          ...
          <section className="bg-slate-800/50 p-6 rounded-3xl border border-slate-700/70 shadow-xl">
            ...
            <div className="w-full overflow-hidden mt-2">
              <div className="relative pt-2 pb-6 px-1">
                ...
                <div className="absolute inset-x-0 top-0 bottom-14 flex flex-col justify-between pointer-events-none">
                  ...
                  {modelComparisons.map((model) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={`text-[10px] font-bold font-mono mb-1.5 transition-colors ${
      ...
      {/* Footer corporativo */}
      <footer className="max-w-7xl mx-auto mt-12 pt-6 border-t border-slate-800 text-center text-xs text-slate-500">
